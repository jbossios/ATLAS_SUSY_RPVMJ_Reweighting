...
def get_data(file_name: str, nepochs: int, batch_size: int = 2048, debug: bool = False): # FIXME add return type hint
  ...
  with h5py.File(file_name, 'r') as hf:
    ...
    for iepoch in range(nepochs): # loop over batches
      ...
      flag_sample = np.random.choice(np.linspace(0, 1, 2), batch_size, p = p_flag)
      ...
      ht_zq_sample = np.random.choice(bin_centers, zq_size, p = np.multiply(p_zq, np.tile([bin_width], p_zq.size)))
      zq_flags = np.tile([1], zq_size)
      ht_nzq_sample = np.random.choice(bin_centers, nzq_size, p = np.multiply(p_nzq, np.tile([bin_width], p_nzq.size)))
      ...
      Batches.append(ht_sample_shaped)
      random.seed(seed)
      random.shuffle(flags_sample_shaped)
      Flags.append(flags_sample_shaped)
      if not iepoch and debug: # compare sampled data for first epoch
        ...
        plt.show()
  return np.array(Batches), np.array(Flags)

if __name__ == '__main__':
  X, y = get_data('/eos/atlas/atlascerngroupdisk/phys-susy/RPV_mutlijets_ANA-SUSY-2019-24/reweighting/Jona/H5_files/v1/mc16a_dijets_JZAll_for_reweighting.h5', 1000, 100000, True)
  print(f'{X[0] = }')
  print(f'{y[0] = }')
